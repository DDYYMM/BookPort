...
class GroupChannelChattingViewController: UIViewController, SBDConnectionDelegate, SBDChannelDelegate, ChattingViewDelegate, MessageDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        for member in groupChannel.members! {
            if let user = member as? SBDUser {
                if user.nickname != UserDefaults.standard.string(forKey: Constants.nameKey) {
                    title += user.nickname!
                }
            }
        }
        self.navItem.title = title.trimmingCharacters(in: NSCharacterSet.whitespaces)
    ...
    @objc private func openMoreMenu() {
        ...
        let seeMemberListAction = UIAlertAction(title: "See member list", style: UIAlertActionStyle.default) { (action) in
            DispatchQueue.main.async {
    ...
    func clickMessage(view: UIView, message: SBDBaseMessage) {
        ...
        if message is SBDUserMessage {
            ...
            if type.hasPrefix("video") {
                openFileAction = UIAlertAction(title: "Play video", style: UIAlertActionStyle.default, handler: { (action) in
                ...
                openFileAction = UIAlertAction(title: "Play Audio", style: UIAlertActionStyle.default, handler: { (action) in
                ...
                openFileAction = UIAlertAction(title: "Open image on Safari", style: UIAlertActionStyle.default, handler: { (action) in
